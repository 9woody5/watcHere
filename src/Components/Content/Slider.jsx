...
const Slider = ({ content }) => {
  ...
  return (
    ...
    >
      ...
      {isHovered && (
        <div
          className={`additional_info w-full h-full absolute top-0 left-0 right-0 bottom-0 bg-zinc-800 text-white flex flex-col items-center justify-end pb-8 bg-opacity-60 px-6 transition-opacity duration-300 ease-in-out opacity-0 hover:opacity-100 rounded-lg border-solid`}
        >
          <h3>{content.title}</h3>
          <p>{content.name}</p>
